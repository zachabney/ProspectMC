...
/**
 * A DamageSnapshot holds data of a specific LivingEntity at the time of
 * attacking
 *
 * @author Codisimus
 */
public class DamageSnapshot {
    ...
    private EnumMap<Attribute, Double> attributeValues = new EnumMap<Attribute, Double>(
            Attribute.class); // A snapshot of relevent attribute values at the
    // time
    private double weaponEfficiency; // A snapshot of their efficiency at the
    // time
    ...
    public DamageSnapshot(LivingEntity entity) {
        ...
        weaponEfficiency = AttributeAPI.getValue(entity,
    ...
    void applyDamage(LivingEntity entity) {
        ...
        if (attributeValues.containsKey(Attribute.ICE_DAMAGE)) {
            damage = (attributeValues.get(Attribute.ICE_DAMAGE) - AttributeAPI
                    .getValue(entity, Attribute.ICE_RESISTANCE)) / 5;
            if (damage > 0) {
                DamageAPI.registerDamageCause(this.entity, DamageCause.MELTING,
                                              entity, 1);
                EntityDamageByEntityEvent event = new EntityDamageByEntityEvent(
                        this.entity, entity,
                        EntityDamageByEntityEvent.DamageCause.MELTING, damage);
        ...
        for (Attribute attribute : attributeValues.keySet()) {
            ...
            if (attribute.hasResistance()) {
                damage -= AttributeAPI.getValue(entity,
                                                attribute.getResistance());
            }
            // Roll to see if the effect occurs
            if (damage > 0 && armorEffeciency < Math.random()
                    && weaponEfficiency > Math.random()) {
                int duration = (int) damage * 20;
                if (attribute.hasPotionEffect()) {
                    PotionEffect effect = attribute
                            .getPotionEffect(duration, 1);
                    entity.addPotionEffect(effect, true);
                    DamageCause cause = getDamageCause(effect);
                    if (cause != null) {
                        DamageAPI.registerDamageCause(this.entity, cause,
                                                      entity, duration);
                    }
                } else {
                    if (attribute == Attribute.FIRE_DAMAGE) {
                        entity.setFireTicks(duration);
                        DamageAPI.registerDamageCause(this.entity,
                                                      DamageCause.FIRE_TICK, entity, duration);
                    }
                }
            }
        }
